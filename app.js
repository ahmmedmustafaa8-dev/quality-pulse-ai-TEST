...
// تم تصليح القفلة هنا في دالة الـ csv
function csv(v){const text=String(v??'');return /^[=+\-@]/.test(text)?`'${text}`:text;}
...
function agentStats(summary, failedRows, calls) {
    ...
    return [...map.values()].map(a => {
        ...
        return {
            ...a,
            average: a.scores.length ? a.scores.reduce((x,y)=>x+y,0)/a.scores.length : calcAvg(a.softScores),
